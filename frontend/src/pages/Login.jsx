...
const Login = () => {
  ...
  return (
    <div className='w-full h-[100vh] bg-cover flex justify-center items-center' style={{ backgroundImage: `url(${bg})` }}>
      <form onSubmit={handleLogin} className='w-[90%] h-[600px] px-[20px] max-w-[500px] bg-[#00000069] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-[20px] '>
        ...
        <button disabled={loading} type='submit' className='min-w-[150px] mt-[30px] h-[60px] bg-white rounded-full text-black font-semibold text-[19px]'>{loading ? "Loading..." : "Login"}</button>
        <p className='text-white text-[19px] cursor-pointer'>Doesn't have an account? <span onClick={() => navigate('/signup')} className='text-blue-400'>SignUp here!</span></p>
      </form>
    </div>
  )
}
